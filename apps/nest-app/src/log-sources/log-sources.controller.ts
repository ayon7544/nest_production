import {
  Controller,
  Get,
  Post,
  Body,
  Patch,
  Param,
  Delete,
} from '@nestjs/common';
import { LogSourcesService } from './log-sources.service';
import { CreateLogSourceDto } from './dto/create-log-source.dto';
import { UpdateLogSourceDto } from './dto/update-log-source.dto';
import { CurrentUser } from '../auth/current-user.decorator';
import type { ICurrentUser } from '../auth/current-user.interface';

@Controller('log-sources')
export class LogSourcesController {
  constructor(private readonly logSourcesService: LogSourcesService) {}

  @Post()
  create(
    @Body() createLogSourceDto: CreateLogSourceDto,
    @CurrentUser(CurrentUser) currentUser: ICurrentUser,
  ) {
    return this.logSourcesService.create(
      createLogSourceDto,
      String(currentUser.id),
    );
  }

  @Get()
  findAll(@CurrentUser(CurrentUser) currentUser: ICurrentUser) {
    return this.logSourcesService.findAll(String(currentUser.id));
  }

  @Get(':id')
  findOne(
    @Param('id') id: string,
    @CurrentUser(CurrentUser) currentUser: ICurrentUser,
  ) {
    return this.logSourcesService.findOne(id, String(currentUser.id));
  }

  @Patch(':id')
  update(
    @Param('id') id: string,
    @Body() updateLogSourceDto: UpdateLogSourceDto,
    @CurrentUser(CurrentUser) currentUser: ICurrentUser,
  ) {
    return this.logSourcesService.update(
      id,
      updateLogSourceDto,
      String(currentUser.id),
    );
  }

  @Delete(':id')
  remove(
    @Param('id') id: string,
    @CurrentUser(CurrentUser) currentUser: ICurrentUser,
  ) {
    return this.logSourcesService.remove(id, String(currentUser.id));
  }
}
